...
The average steps taken per day is `r daymean` (yellow line in histogram).  
The median number of steps taken per day is `r daymedian` (purple line in histogram).
...
The maximum number of steps on average for a given 5-minute interval is `r max`, which corresponds to the `r int1` interval.
...
```{r replacing missing values for the mean corresponding to that 5-minute interval}
...
The corrected average of steps taken per day is `r newdaymean` (yellow line in histogram).  
The corrected median number of steps taken per day is `r newdaymedian` (purple line in histogram).  
